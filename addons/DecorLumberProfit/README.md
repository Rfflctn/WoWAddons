...
10. Цены и количество предметов на АХ сохраняются отдельно по каждому реалму (сбор идёт всегда, в фоне).
11. Тултип готового предмета показывает цену, общую конкуренцию и свои лоты на текущем сервере.
12. Колонка «Мои» показывает сумму лотов всех персонажей аккаунта на текущем реалме и отключается через «Столбцы».
13. TOC + Lua + документация, без Lua-ошибок при отсутствии данных.
...
4. Откройте тултип строки — он покажет цену предмета, конкуренцию и ваши лоты.
5. Для актуализации своих лотов войдите каждым персонажем и откройте аукцион; Blizzard API не позволяет читать текущие лоты офлайн-персонажей.
6. Галочка «Скрыть неизученное» (справа сверху) — скрывает рецепты, не изученные ни на одном персонаже аккаунта (учитывается общая база `learnedBy`); настройка сохраняется между сессиями. Режим «Топ выгода» фильтрует так же.
...
| Цены аукциона | Нет единого `GetReagentPrice`. Только поштучные `SendSearchQuery` → `GetCommoditySearchResultInfo`/`GetItemSearchResultInfo` с throttle 100/мин и требованием `IsThrottledMessageSystemReady`. Полный скан АХ возможен только через `ReplicateItems` (весь аукцион одним вызовом) — для набора предметов аддона избыточен. | Очередь с `QUERY_DELAY 0.62s` (упор в лимит 100/мин), сортировка `SendSearchQuery` по цене; для реагентов-commodity — средняя цена 10 самых дешёвых единиц (лоты по количеству), для крафтового предмета — минимум; быстрый предварительный `SendBrowseQuery` по категориям (Reagent/Tradegoods/Housing) показывает приблизительную цену «~», пока фоном идёт точный скан. Обработка событий `COMMODITY_SEARCH_RESULTS_UPDATED`/`ITEM_SEARCH_RESULTS_UPDATED`, TTL свежести 1 ч (гейтит только рескан: протухшая цена продолжает показываться, пока идёт обновление), fallback к `GetBrowseResults` через O(1)-индекс. Последняя известная цена (включая пометку «нет на АХ» с сохранённой ценой) хранится в SavedVariables до ручного сброса (`/dlp reset`, кнопка «Очистить кэш») или переустановки. Древесина на АХ не ищется (не продаётся; только кэш для колонки «цена др.»). Честно показывает «нет на АХ» / «цена неизвестна» только если цены не было никогда. |
