...
def run(lua, check, exec_):
    ...
    check('addon version', 'DecorLumberProfit.VERSION', '2.0.0')
    check('addon db schema v1', 'DecorLumberProfit.DB_SCHEMA', '1')
    ...
    check('diag version tracks init', 'tostring(string.find(DecorLumberProfitDiag.ADDON_VERSION, "2.0.0", 1, true) ~= nil)', 'true')
